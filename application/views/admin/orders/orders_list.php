							...
							<div class="card-body table-responsive p-0">
								<table class="table table-hover text-nowrap">
									...
									<tr>
										...
										<th>
											<select id="status-select" class="form-control col-md-8">
												...
												<option <?= $status == -1 ? 'selected' : '' ?> value="-1"> Ləğv edilmiş </option>
									...
									<?php foreach ($orders as $order):
										...
										?>
										<tr style="cursor: pointer"  onclick="window.location='<?=base_url().'admin/order/order_details/'.$order['id']?>';">
											<td> <?=$order['id']?> </td>
											...
											<td onclick="event.stopPropagation(); return false;">
												<select class="form-control status-change" data-order-id="<?=$order['id']?>">
													<option <?= $order['status'] == -1 ? 'selected' : '' ?> value="-1"> Ləğv edilmiş  </option>
...
<script>
	$("#status-select").on('change', function () {
			let status = $(this).val();
			window.location.href = "<?=base_url()?>admin/order/index/" + status + "";
	});

	$(".status-change").on('change', function () {
		let status = $(this).val();
		let order_id = $(this).data('order-id');
		window.location.href = "<?=base_url()?>admin/order/change_status/" + status + "/" + order_id;
	});
